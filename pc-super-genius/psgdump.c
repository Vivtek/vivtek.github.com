...
/* Oh -- its *purpose*?  Well, it dumps the contact data from psg1.dat of Ruben
   Garcia's PC-Super-Genius program.  It took me two hours to figure out the
   file structure and forty minutes to write this program and test it against
   a single copy of psg1.dat.
...
int main(int argc, char *argv[])
{
   ...
   if (argc == 1) { strcpy (filename, "psg1.dat"); } else { strcpy (filename, argv[1]); }
   ...
   for (i=0; i < records; i++)
   {
      ...
      readfield (25, 0x65 + i); /* Now the "message to your users." */
      printf ("%s\t", field);

      /* Now let's get the other fields in case we want to figure out what they are.
         No trick for these. */
      readfield (4, 0x65 + i);  printf ("%s\t", field);
      readfield (1, 0x65 + i);  printf ("%s\t", field);
      readfield (1, 0x65 + i);  printf ("%s\t", field);
      readfield (4, 0x65 + i);  printf ("%s\t", field);
      readfield (8, 0x65 + i);  printf ("%s\t", field);
      readfield (8, 0x65 + i);  printf ("%s\t", field);
      readfield (8, 0x65 + i);  printf ("%s\t", field);
      readfield (10, 0x65 + i);  printf ("%s\t", field);
